...
// Full version: TOWSOTH Name Slider Section for Modern Landing Page
// Use directly above “The Struggle Is Real” section in ModernLandingPage.jsx
// import TowsothNameSliderSection from '@/components/TowsothNameSliderSection';

export default function TowsothNameSliderSection() {
  return (
    <section className="responsive-padding py-12 sm:py-16 bg-gradient-to-r from-blue-600/10 to-purple-600/10">
      <div className="container mx-auto text-center">
        ...
        <div className="max-w-3xl mx-auto">
          <NameSlider
            initialNames={["Aarav Sharma", "Isha Mehta", "Kabir Singh", "Diya Kapoor", "Vivaan Gupta", "Anaya Joshi"]}
            intervalMs={2500}
          />
        </div>
      </div>
...
// Inner Slider Component (fully functional)
function NameSlider({ initialNames = [], intervalMs = 2500 }) {
  ...
  useEffect(() => {
    if (!playing || names.length <= 1) return;
  ...
  const variants = {
    ...
    center: { x: 0, opacity: 1, filter: "blur(0px)", transition: { type: "spring", stiffness: 280, damping: 30 } },
    exit: (dir) => ({ x: dir > 0 ? -60 : 60, opacity: 0, filter: "blur(6px)", transition: { duration: 0.18 } }),
  };

  return (
    <div className="w-full bg-card border border-border rounded-3xl shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        <button onClick={prev} className="rounded-lg p-2 border hover:bg-muted transition">
          <ChevronLeft />
        </button>
        <div className="relative h-20 flex items-center justify-center w-full">
          <AnimatePresence initial={false} custom={direction}>
            <motion.div
              key={`${safeIndex}-${names[safeIndex]}`}
              custom={direction}
              variants={variants}
              initial="enter"
              animate="center"
              exit="exit"
              className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-to-r from-blue-500 to-purple-400 bg-clip-text text-transparent"
            >
              {names.length ? names[safeIndex] : "Add names to begin"}
            </motion.div>
          </AnimatePresence>
        </div>
        <button onClick={next} className="rounded-lg p-2 border hover:bg-muted transition">
          <ChevronRight />
        </button>
      </div>
      ...
      <div className="flex justify-center gap-2 mb-4">
        {names.map((_, i) => (
          <button
            key={i}
            onClick={() => setIndex(i)}
            className={`h-2.5 w-2.5 rounded-full transition ${i === safeIndex ? "bg-blue-600 scale-110" : "bg-gray-300 hover:bg-gray-400"}`}
          />
        ))}
      </div>
      ...
      <div className="flex gap-2 mt-4 justify-center">
        <input
          ...
          className="border rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400 outline-none w-2/3"
        />
        <button onClick={addName} className="flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-xl text-sm">
          <Plus size={14} /> Add
        </button>
        <button onClick={() => setPlaying((p) => !p)} className="flex items-center gap-1 border px-3 py-2 rounded-xl text-sm">
      ...
      <div className="flex flex-wrap gap-2 mt-4 justify-center">
        {names.map((n, i) => (
          <span key={i} className="flex items-center gap-2 bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-sm">
            {n}
            <button onClick={() => removeName(i)} className="text-red-500 hover:text-red-700">
              <X size={14} />
            </button>
          </span>
        ))}
